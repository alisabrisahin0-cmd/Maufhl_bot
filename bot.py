# MAC ANALIZ BOTU - V13.5-CORE
# Hedef: Maç kaçırmayı durdurmak için filtreler sadeleştirildi.
...
async def get_ai_commentary(ev, dep, dk, skor, sot, da_ev, da_dep, lig):
    global key_index
    if not HAS_GENAI: return "⚠️ AI Yüklenmedi."
    try:
...
async def analiz_et(mac_detay, m_id):
    ...
    # --- SADELEŞTİRİLMİŞ FİLTRE ---
    # Sadece 20-85 dk arasını kabul et, geri kalan tüm engelleri kaldır.
    if not (20 <= dk <= 85): 
        return None
    ...
    puan = 4.0 # Zaman puanı (20-85 dk arası her maç alır)
    ...
    # 3 fark ve üzeri ise ele (Maç bitmiş sayılır)
...
async def ana_dongu():
    ...
    await bot.send_message(chat_id=CHAT_ID, text="✅ V13.5 CORE AKTİF: Tüm maçlar taranıyor.")
    async with aiohttp.ClientSession() as session:
        while True:
            try:
                # inplay endpoint'inden tüm aktif maçları çek
                async with session.get(f"https://api.betsapi.com/v3/bet365/inplay?token={BETSAPI_TOKEN}") as r:
                    data = await r.json()
                    res = data.get('results', [])
                
                logger.info(f"📡 {len(res)} maç taranıyor...")
                for m in res:
