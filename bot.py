# MAC ANALIZ BOTU - V13.3-RTC
# Yenilik: Donmuş Dakika Kontrolü (Stale Data Protection) ve Zaman Doğrulama.
...
async def get_ai_commentary(ev, dep, dk, skor, sot, da_ev, da_dep, lig):
    global key_index
    if not HAS_GENAI: return "⚠️ AI Kütüphanesi (google-genai) yüklü değil."
    try:
        current_key = GEMINI_KEYS[key_index % len(GEMINI_KEYS)]
        key_index += 1
        client = genai.Client(api_key=current_key)
        prompt = (f"Futbol Analisti: {ev} {skor} {dep} | Dk: {dk}. "
                  f"İst: SOT:{sot}, DA:{da_ev}-{da_dep}. Taktiksel risk analizi yap.")
...
async def analiz_et(mac_detay, m_id):
    ...
    # --- DONMUŞ DAKİKA KONTROLÜ ---
    # Eğer skor 0-1 iken dakika hala 45 görünüyorsa ama maç gerçekte 80'deyse, 
    # API genellikle 'TT' (Timer Type) veya 'TS' (Timer Status) bilgisini günceller.
    # Biz burada 'TM' verisinin mantıksal sınırda kalıp kalmadığına bakıyoruz.
    if not (20 <= dk <= 85): 
        return None 
...
async def ana_dongu():
    ...
    await bot.send_message(chat_id=CHAT_ID, text="✅ V13.3 RTC AKTİF: Gerçek zamanlı dakika kontrolü devrede.")
    async with aiohttp.ClientSession() as session:
        while True:
            try:
                # inplay_filter yerine doğrudan aktif maçları çeken ana listeyi kullanıyoruz
                ...
                for m in res:
                    ...
                    # Maçın zamanlayıcısını (timer) kontrol et
                    timer = m.get('timer', {})
                    su_anki_dk = safe_int(timer.get('tm', 0))
                    
                    # Eğer timer durmuşsa veya 45/90'da takılıysa analiz etme
                    if timer.get('ts') == 0: continue 
                    ...
                    async with session.get(f"https://api.betsapi.com/v3/bet365/event?token={BETSAPI_TOKEN}&FI={m_id}&stats=1") as er:
                        ...
                        if e_data.get('success') == 1:
                            msg = await analiz_et(e_data['results'][0], m_id)
                            if msg:
