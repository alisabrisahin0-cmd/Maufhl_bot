...
async def analiz_et(mac_detay, m_id):
    ...
    if (ev_gol, dep_gol) in onayli_skorlar:
        puan += 3.0
        detaylar.append(f"🎯 Skor ({skor}): +3.0")
        if (ev_gol, dep_gol) == (1,1): puan += 1.0; detaylar.append("⭐ 1-1 Bonusu: +1.0")
    ...
    if delta_atak >= 5:
        puan += 2.0; detaylar.append(f"🚀 Atak İvmesi (Δ:{delta_atak}): +2.0")
    ...
    if puan >= 3.0:
        ...
        return {
            "mesaj": (f"💎 SİNYAL & AI ANALİZ (Puan: {puan})\n⚽ {ev_adi} {skor} {dep_adi}\n"
                      f"⏱ Dakika: {dk} | Lig: {lig}\n--------------------\n"
                      f"🤖 AI YORUMU:\n_{ai_yorum}_\n\n📊 DETAY:\n"
                      f"🚀 Atak (DA): {ev_da} - {dep_da} | SOT: {toplam_sot}\n--------------------\n"
                      f"🔗 [Nesine'de Maçı Ara]({nesine_link})")
        }
    return None
...
async def ana_dongu():
    ...
    async with aiohttp.ClientSession() as session:
        while True:
            ...
            except Exception as e: logger.error(f"Sistem Hatası: {e}")
