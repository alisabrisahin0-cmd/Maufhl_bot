# MAC ANALIZ BOTU - V8.4 THE TRUE ORACLE (GERÇEK KAHİN)
# Özellikler: 
# 1- JSON Çökmesi Tamamen Kaldırıldı (Düz Metin Özgür AI)
# 2- Klişe Yasaklandı: Sizi net yönlendiren gerçek yorumlar
# 3- Ghost Match ve Devre Arası Koruması
# 4- Güvenli Kod Formatı ve 3x API Rotasyonu
            ...
            async with session.get(list_url, timeout=20) as resp:
                ...
                for f in adaylar:
                    ...
                    if detay and isinstance(detay, list):
                        try:
                            ev_isim = "Ev Sahibi"; dep_isim = "Deplasman"; lig_isim = "Bilinmiyor"
                            dk = 0; tt = '1'; ev_gol = 0; dep_gol = 0
                            ev_korner = 0; dep_korner = 0; ev_kirmizi = 0; dep_kirmizi = 0
                            ...
                            for i, item in enumerate(detay):
                                ...
                                if t == 'EV':
                                    ...
                                    if match:
                                        ...
                                        if olay_dakikasi > dk: dk = olay_dakikasi
...
def sinyal_hesapla(mac):
    ...
    if gecmis['dakika'] == dakika:
        ...
        if gecen_sure > 15 and not mac.get('devre_arasi', False): return 0, [], "GHOST"
    else:
...
def tavsiye_uret(mac, strateji):
    ...
    if strateji == "KORNER_BASKISI":
        if ev_korner > dep_korner and korner_farki >= 2: return "🎯 KESİN TAHMİN: SIRADAKİ GOLÜ EV SAHİBİ ATAR", "Ev sahibi kornerlerle oyunu rakip sahaya yıktı."
        elif dep_korner > ev_korner and korner_farki >= 2: return "🎯 KESİN TAHMİN: SIRADAKİ GOLÜ DEPLASMAN ATAR", "Deplasman ekibi baskıyı artırdı, savunma bunaldı."
        return "🎯 KESİN TAHMİN: SIRADAKİ GOL OLUR (ÜST)", "İki takım da karşılıklı korner üretiyor, maç git-gelli."
    
    elif strateji == "KIRMIZI_KART": return "🎯 KESİN TAHMİN: SIRADAKİ GOL OLUR (ÜST)", "Kırmızı kart oyunu tamamen açtı, savunma dengesi kayboldu."
    elif strateji == "GOL_PATLAMASI": return "🎯 KESİN TAHMİN: MAÇ SONU ÜST BİTER", "Maçta gol düellosu var, takımlar savunmayı bırakmış."
    
    if gol_fark == 0: 
        if ev_korner > dep_korner: return "🎯 KESİN TAHMİN: SIRADAKİ GOLÜ EV SAHİBİ ATAR", "Skor eşit ama ev sahibi galibiyete daha yakın."
        elif dep_korner > ev_korner: return "🎯 KESİN TAHMİN: SIRADAKİ GOLÜ DEPLASMAN ATAR", "Skor eşit ama deplasman ciddi tehlike yaratıyor."
        return "🎯 KESİN TAHMİN: 0.5 ÜST (Maçta Gol Olur)", "Saha içi aksiyon yüksek, takımlar beraberliğe razı değil."
    
    if gol_fark > 0: return "🎯 KESİN TAHMİN: MAÇ SONU EV SAHİBİ KAZANIR", "Ev sahibi önde ve momentumu hala koruyor."
    return "🎯 KESİN TAHMİN: MAÇ SONU DEPLASMAN KAZANIR", "Deplasman ekibi üstünlüğü ele aldı, rakibi çıkartmıyor."

# ================================================
# GEMİNİ AI (V8.4 - DÜZ METİN ÖZGÜR YORUM MOTORU)
# ================================================
...
async def gemini_analiz(mac, tahmin, neden):
    ...
    prompt = f"""Sen efsanevi, maçın kaderini okuyan bir canlı bahis üstadısın. 
Bize YÖN VER! Botun tahmininin mantıklı olup olmadığını iddialı bir dille anlat.

MAÇ BİLGİSİ: {mac['ev']} {mac['ev_gol']}-{mac['dep_gol']} {mac['dep']} ({mac['dakika']}. Dakika)
KORNER: {mac['ev_korner']} - {mac['dep_korner']}
KART (Kırmızı): {mac['ev_kirmizi']} - {mac['dep_kirmizi']}
BOTUN SİSTEM TAHMİNİ: {tahmin}

KURAL 1: Klişe kelimeler kullanma. Maçtaki rakamları (korner sayısı, skor, dakika) metin içinde ASLA TEKRARLAMA! Zaten yukarıda yazıyor.
KURAL 2: Maç 0-0'a kilitlenmeye müsaitse veya çok riskliyse bizi UYAR!
KURAL 3: Sadece 2 veya 3 cümlelik, iddialı, yönlendirici bir analiz yaz.
KURAL 4: Eğer maça girmemizi ÖNERMİYORSAN yorumunun sonuna tam olarak [RİSKLİ] yaz. Öneriyorsan [UYGUN] yaz. Başka hiçbir yapısal format (JSON vb.) KULLANMA. Sadece düz metin yaz."""
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(url, json=payload, timeout=15) as resp:
                    if resp.status == 200:
                        ...
                        gir = True
                        if "[RİSKLİ]" in text.upper() or "[RISKLI]" in text.upper():
                            gir = False
                        
                        text = text.replace("[UYGUN]", "").replace("[RİSKLİ]", "").replace("[uygun]", "").replace("[riskli]", "").replace("[RISKLI]", "").strip()
    ...
    # Hata durumunda bile artık ezbere tek cümle yok, maça özel dinamik fallback
    return f"{mac['ev']} - {mac['dep']} mücadelesinde tempo artmış durumda, savunma kurguları her an hata vermeye müsait görünüyor.", True
...
async def bildirim_gonder(bot, mac, puan, detay, tahmin, neden, ai_yorum, ai_onay, strateji):
    ...
    if mac.get('devre_arasi'): dk_gosterimi = "45. DK (Devre Arası)"

    if not ai_onay:
        mesaj = f"⚠️ NESİNE RİSK UYARISI — İŞLEME GİRME!\n{mac['ev']} {mac['ev_gol']}-{mac['dep_gol']} {mac['dep']} | {dk_gosterimi}\n📺 BÜLTEN: {nesine_durum}\n🕵️‍♂️ KAHİN AI: {ai_yorum}"
    ...
    mesaj = (
        ...
        f"────────────────────\n"
        f"🕵️‍♂️ KAHİN AI (YÖN GÖSTERİCİ) YORUMU:\n{ai_yorum}\n"
        f"────────────────────\n"
        f"💡 {tahmin}\n"
        f"📌 GEREKÇE: {neden}\n"
        f"{'═'*20}\n{karar_emoji} {karar}\n{'═'*20}"
...
def sonuc_kontrol(tahmin, bas_ev, bas_dep, fin_ev, fin_dep):
    yeni_ev = fin_ev - bas_ev; yeni_dep = fin_dep - bas_dep; toplam = yeni_ev + yeni_dep
    tahmin_upper = tahmin.upper()
    
    if "ÜST" in tahmin_upper or "SONRAKİ GOL" in tahmin_upper or "GOL OLUR" in tahmin_upper: return "TUTTU" if toplam >= 1 else "DSTU"
    elif "EV SAHİBİ ATAR" in tahmin_upper or "EV SAHİBİ KAZANIR" in tahmin_upper: return "TUTTU" if yeni_ev >= 1 or fin_ev >= fin_dep else "DSTU"
    elif "DEPLASMAN ATAR" in tahmin_upper or "DEPLASMAN KAZANIR" in tahmin_upper: return "TUTTU" if yeni_dep >= 1 or fin_dep > fin_ev else "DSTU"
    return "BELIRSIZ"
...
async def ana_dongu():
    ...
    try: await bot.send_message(chat_id=CHAT_ID, text="🤖 V8.4 THE TRUE ORACLE — AKTİF\n✅ JSON Çökmesi Giderildi\n✅ %100 Yönlendirici (Düz Metin) Yapay Zeka Devrede\n\nGözlem Başlıyor...")
    except: pass
    ...
    while True:
        try:
            ...
            for mac in maclar:
                ...
                mac_id = mac['id']
                if strateji == "GHOST" or strateji == "GENEL": continue

                if mac_id in bildirim_gonderilen:
                    biten_maclar[mac_id] = { 'ev': mac['ev'], 'dep': mac['dep'], 'tahmin': bildirim_gonderilen[mac_id]['tahmin'], 'bas_ev': bildirim_gonderilen[mac_id]['ev_gol'], 'bas_dep': bildirim_gonderilen[mac_id]['dep_gol'], 'son_ev': mac['ev_gol'], 'son_dep': mac['dep_gol'] }
                ...
                if puan >= MIN_PUAN:
                    ...
                    if puan > onceki:
                        ...
                        bildirim_gonderilen[mac_id] = {'puan': puan, 'tahmin': tahmin, 'ev_gol': mac['ev_gol'], 'dep_gol': mac['dep_gol']}
