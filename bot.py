...
class LeagueFilter:
    ...
    def get_league_multiplier(league_name: str) -> float:
        ...
        🆕 V46: Makale bulguları ile iyileştirildi
        - Türkiye Süper Lig eklendi (yüksek tempo)
        - Portekiz U23 özel bonus (mükemmel kalibrasyon)
        - Düşük tempo ligler için ceza çarpanı
        
        Returns:
            float: 0.7 - 1.6 arası çarpan
        """
        league_lower = league_name.lower()
        
        # Portekiz U23 özel durum (1.6x) - Makale: "Mükemmel kalibrasyon"
        if 'portugal' in league_lower and 'u23' in league_lower:
            return 1.6
        
        # Premium ligler (1.5x) - Makale onaylı yüksek tempo
        premium_keywords = [
            'bundesliga', 'eredivisie', 'premier league',
            'super lig', 'süper lig', 'turkey'  # 🆕 Türkiye eklendi
        ]
        if any(k in league_lower for k in premium_keywords):
            return 1.5
        
        # U23/Gençlik ligleri (1.4x) - Makale: "Yüksek volatilite"
        if any(k in league_lower for k in ['u23', 'u21', 'u20', 'youth']):
            return 1.4  # 1.3'ten yükseltildi
        
        # İyi ligler (1.2x)
        if any(k in league_lower for k in ['championship', 'serie a', 'la liga']):
            return 1.2
        
        # 🆕 Düşük tempo ligler (0.7x) - Makale: "Statik rejim"
        low_tempo_keywords = ['kuwait', 'egypt', 'third division', 'regionalliga']
        if any(k in league_lower for k in low_tempo_keywords):
            return 0.7
    ...
    def get_da_threshold(league_name: str) -> float:
        """
        🆕 V46: Lig bazlı DA momentum eşiği
        
        Makale bulgusu: "Her lig farklı rejim sergiler"
        - Yüksek tempo: DA ≥ 1.3 (Bundesliga, Eredivisie, U23, Türkiye)
        - Düşük tempo: DA ≥ 2.0 (Kuwait, Egypt)
        - Standart: DA ≥ 1.5
        
        Returns:
            float: DA/dakika minimum eşiği
        """
        league_lower = league_name.lower()
        
        # YÜKSEK TEMPO LİGLER (Düşük eşik - daha agresif)
        high_tempo_keywords = [
            'bundesliga', 'eredivisie', 'premier league',
            'u23', 'u21', 'u20', 'u19',
            'super lig', 'süper lig', 'turkey', 'portugal'
        ]
        if any(k in league_lower for k in high_tempo_keywords):
            return 1.3  # Makale önerisi
        
        # DÜŞÜK TEMPO LİGLER (Yüksek eşik - daha konservatif)
        low_tempo_keywords = [
            'kuwait', 'egypt', 'third division',
            'regionalliga', 'amateur'
        ]
        if any(k in league_lower for k in low_tempo_keywords):
            return 2.0  # Makale önerisi
...
class IYGolModule:
    ...
    def check(minute: int, home_score: int, away_score: int,
        ...
        # 🆕 MAKALE İYİLEŞTİRMESİ: Altın pencere ÇARPANI (24-36 dk)
        # Makale bulgusu: "24-36 dakika arası %100 başarı oranı"
        # Eski: score += 3.0 (toplama)
        # Yeni: score *= 1.8 (çarpan - %80 bonus)
        reason = "İY Gol sinyali - DA ivmesi yüksek"
        if 24 <= minute <= 36:
            score *= 1.8  # Makale önerisi: %80 bonus
            reason += " | ALTIN PENCERE (1.8x)"
...
class EvDepGolModule:
    ...
    def check(minute: int, home: TeamStats, away: TeamStats,
        ...
        # 🆕 MAKALE İYİLEŞTİRMESİ: AH Anomali Bonusu
        # Makale bulgusu: "İade olmayan AH çizgilerinde halk sistematik kaybeder"
        # İade OLMAYAN: -1.0, -2.0, -3.0 (yüksek bonus)
        # İade OLAN: -0.75, -1.25, -1.75 (düşük bonus)
        ah_value = abs(ah_home) if dominant_team == "HOME" else abs(ah_away)
        reason = f"{dominant_team} baskın - AH değerli çizgi"
        
        if ah_value in [1.0, 2.0, 3.0]:  # İade olmayan çizgiler
            score += 3.0  # Yüksek bonus
            reason += " | AH ANOMALİ (iade yok)"
        elif ah_value in [0.75, 1.25, 1.75]:  # İade olan çizgiler
            score += 1.5  # Düşük bonus
            reason += " | AH (iade var)"
        elif ah_value >= 1.0:  # Diğer yüksek değerler
            score += 2.0
...
class IY2Module:
    ...
    def check(minute: int, home: TeamStats, away: TeamStats,
        ...
        🆕 V46: Gelişmiş rölanti tespiti eklendi
        """
        ...
        # 🆕 V46: GELİŞMİŞ RÖLANTI TESPİTİ
        # Makale: "Kuveyt liginde 3-0 sonrası momentum sıfırlandı"
        score_diff = abs(home_score - away_score)
        
        # Kural 1: Büyük skor farkı
        if score_diff >= 3:
            return SignalResult(False, None, 0.0, "Rölanti: Büyük skor farkı (≥3)", {})
        
        # Kural 2: Orta skor farkı + geç dakika (oyun yönetimi)
        if score_diff >= 2 and minute >= 75:
            return SignalResult(False, None, 0.0, "Rölanti: Oyun yönetimi (2+ fark, 75+ dk)", {})
        ...
        # Kural 3: Düşük momentum tespiti (yeni)
        # Makale: "Momentum metrikleri sıfırlanmış"
        if minute >= 60 and da_per_minute < 0.8:
            return SignalResult(False, None, 0.0,
                f"Rölanti: Düşük momentum ({da_per_minute:.2f} DA/dk)", {})
        ...
        # 🆕 MAKALE İYİLEŞTİRMESİ: Kırılma evresi ÇARPANI (48-58 dk)
        # Makale bulgusu: "48-58 dakika kırılma evresi - savunma organize değil"
        # Eski: score += 5.0 (toplama)
        # Yeni: score *= 2.0 (çarpan - %100 bonus)
        reason = f"İY2 {window} pencere - DA momentum yüksek"
        if 48 <= minute <= 58:
            score *= 2.0  # Makale önerisi: %100 bonus
            reason += " | KIRILMA EVRESİ (2.0x)"
