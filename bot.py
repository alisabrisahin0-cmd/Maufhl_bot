# MAC ANALIZ BOTU - V8.3 THE ORACLE (TAM SÜRÜM)
# Özellikler: 
# 1- Yön Gösteren, Ezber Bozan ve Klişe Kullanmayan Oracle AI
# 2- JSON Format Çökme Koruması (Bulletproof)
# 3- Ghost Match (Donuk Maç) Dedektörü
# 4- Açık Türkçe Nesine Tahminleri
# 5- 429 Limit Koruması (3'lü API Rotasyonu)
...
KURAL 1: Bana klişe ve ezbere laflar etme. Rakamları (skor, korner, dakika) YORUMUN İÇİNDE ASLA TEKRAR ETME.
KURAL 2: Bizi YÖNLENDİR! Maçın gizli hikayesini anlat ve bahsin kaderini belirle.
KURAL 3: Yanıtın KESİNLİKLE JSON formatında olmalıdır.
...
2. Bize net bir yön ver! Örneğin: "Baskı çok gibi dursa da bu maç 0-0'a kilitlenmeye çok müsait, bence uzak durun!" ya da "Korner ablukası savunmanın gardını tamamen düşürmüş, gol adeta geliyorum diyor, botun tahmini tereddütsüz oynanır."
...
SADECE ŞU JSON FORMATINDA YANIT VER (Başka hiçbir kelime ekleme):
{{"yorum": "Senin efsanevi, yol gösterici analizin", "gir": true}}"""
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(url, json=payload, timeout=15) as resp:
                    if resp.status == 200:
                        ...
                        return res.get('yorum', 'Savunma hatlarında boşluklar var, oyunun yönü her an değişebilir.'), res.get('gir', True)
    ...
    return "Maçın taktiksel savaşı kızışmış durumda, istatistikler ve saha içi dinamikler kırılma noktasına işaret ediyor.", True
...
async def ana_dongu():
    ...
    try: 
        await bot.send_message(
            chat_id=CHAT_ID, 
            text="🤖 V8.3 THE ORACLE — AKTİF\n✅ Kahin AI Devrede (Sizi Birebir Yönlendirecek)\n✅ JSON Çökme Koruması\n✅ Ghost Match ve Devre Arası Koruması\n\nGözlem Başlıyor..."
        )
    except Exception as e: 
        pass
