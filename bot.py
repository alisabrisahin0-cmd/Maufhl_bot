"""
MAUFHL_BOT V4.0 QUANT MASTER - V1 COMPATIBILITY EDITION
V3 Yetki Hatalarını Aşmak İçin V1 Uç Noktasına Geçiş Yapıldı.
"""
...
class HealthCheckHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        self.end_headers()
        self.wfile.write(b"V4.0 V1-Engine Active")
...
def aktif_mi():
    simdi = datetime.now()
    # Railway'de TZ=Europe/Istanbul ayarı yapıldığı varsayılmıştır
    return 13 <= simdi.hour <= 23

# ================================================
# V4.0 QUANT MASTER FİLTRELERİ
# ================================================
def sinyal_hesapla(mac):
    ...
    if toplam_gol >= 5:
        return 0, ["🚫 KAOS SINIRI: 5+ gol."], "BLOCKED", False

    fark = dakika - son_gol
    if son_gol > 0 and fark <= 5:
        return 0, [f"🚫 AŞIRI ISINMA: +{fark} dk."], "BLOCKED", False
    ...
    if not ilk_tarama and dakika > 45 and delta_atak < 10 and delta_sut < 1:
        return 0, ["🚫 MOMENTUM TABANI."], "REJECTED", False

    if ilk_tarama:
        detay.append("🔍 İLK ÖLÇÜM ALINIYOR")
        puan += 2.0
    else:
        if delta_atak > 12 and delta_sut == 0:
            puan -= 2.0
            detay.append("⚠️ KISIR BASKI")
        else:
            puan += 4.0
            detay.append(f"✅ İVME: +{delta_atak} Atak")

    if 55 <= dakika <= 60:
        puan += 3.0
        detay.append("🌟 ALTIN PENCERE")
...
async def gemini_analiz(mac):
    ...
    if not valid_keys: return "AI Devre Dışı."

    prompt = f"{mac['ev']} vs {mac['dep']} (Dk:{mac['dakika']}). Maçın ivmesi yüksek. Gidişatı 2 kısa Türkçe cümleyle yorumla."
    ...
    return "Yapay zeka meşgul."

# ================================================
# V1 MOTORU (TRIAL PAKET UYUMLU)
# ================================================
async def maclari_cek():
    maclar = []
    try:
        # V3 YERİNE V1 KULLANILIYOR
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url, timeout=10) as resp:
                ...
                if data.get('success') != 1:
                    logger.error(f"🔴 BetsAPI V1 REDDETTİ: {data}")
                    return maclar
                
                results = data.get('results', [])
                logger.info(f"💎 BetsAPI V1: {len(results)} maç işleniyor...")

                for f in results:
                    try:
                        m_id = str(f.get('id'))
                        # V1'de timer yapısı farklı olabilir, güvenli çekim:
        ...
        logger.error(f"⚠️ V1 Bağlantı Hatası: {e}")
    return maclar
...
async def ana_dongu():
    ...
    start_msg = "🤖 V4.0 QUANT MASTER (V1 ENGINE) BAŞLADI\n\n⚠️ V3 Yetki Hatası Nedeniyle V1 Altyapısına Geçildi."
