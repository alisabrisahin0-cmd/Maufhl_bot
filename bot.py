"""
V4.5 QUANT MASTER - BET365 TRIAL ADAPTIVE (FULL & FINAL)
Özellikler: Two-Step Data Fetch, Rolling Window, Cold Start Fix, Düz Metin AI
"""
...
def sinyal_hesapla(mac):
    ...
    # 2. ROLLING WINDOW & COLD START FIX
    ...
    # İvme Kontrolü (Hard Lock) - İlk taramada uygulanmaz
    if not ilk_tarama and delta_atak < 7 and delta_sut < 1 and dakika > 20:
        return 0, ["HARD LOCK: Yetersiz ivme."], False

    if ilk_tarama:
        detay.append("🔍 KAPI: İlk ölçüm alınıyor (Referans)")
        puan += 2.0 
    else:
        detay.append(f"✅ KAPI GEÇİLDİ: Periyot İvmesi (Atak: +{delta_atak}, Şut: +{delta_sut})")
        puan += 4.0 

    # 3. İSTATİSTİK PUANLAMA
    puan += (suanki_sut * 0.5)
    detay.append(f"🎯 Toplam Şut: {suanki_sut}")
...
async def gemini_analiz(mac):
    ...
    prompt = f"MAÇ: {mac['ev']} {mac['ev_gol']}-{mac['dep_gol']} {mac['dep']} | LİG: {mac['lig']} | DK: {mac['dakika']}\nİvme saptandı. Maçın gidişatını 2 cümle Türkçe yorumla. JSON kullanma."
    ...
    return "Yapay zeka şu an yoğun, istatistiksel ivme devam ediyor."
...
async def mac_detay_cek(session, fixture_id):
    """Her maçın detay odasına gidip istatistikleri çeker"""
    try:
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get(list_url, timeout=10) as resp:
                ...
                logger.info(f"💎 Liste Alındı: {len(raw_results)} maç. Detaylar toplanıyor...")

                # ADIM 2: Her maçın detayına (İstatistiklerine) git
                for f in raw_results:
                    try:
                        ...
                        def gs(key, idx):
                            v = stats.get(key, [0, 0])
                            return int(v[idx]) if isinstance(v, list) and len(v) >
